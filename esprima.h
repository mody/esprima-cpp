...
struct Node : Poolable
{
    ...
    Node(Pool& pool)
        ...
        , groupLoc(NULL)
    {
        range[0] = range[1] = groupRange[0] = groupRange[1] = 0;
    }
    virtual ~Node() {}
...
struct AssignmentExpression : Expression
{
    ...
    Expression* left;
    Expression* right;
    AssignmentExpression(Pool& pool)
        : Expression(pool)
        , left()
        , right()
    {}
...
struct ConditionalExpression : Expression
{
    ...
    ConditionalExpression(Pool& pool)
        : Expression(pool)
        , test()
        , alternate()
        , consequent()
